...
from sklearn.model_selection import StratifiedKFold # added for stratified k-fold cross validation
...
x_con = np.concatenate((x_tr, x_val, x_te), axis=0)
y_con = np.concatenate((y_tr, y_val, y_te), axis=0)

# convert the targets to none-one-hot
y_con_noonehot = [np.argmax(y, axis=None, out=None) for y in y_con]
...
for train, test in skf.split(x_con, y_con_noonehot):
    selections[fold_no] = (train,test)
    model, name = CreateModel(embedding_size, x_con.shape[1], vocab_size, windows, kernels, nb_of_outputs, trainable)
    ...
    history = model.fit(x_con[train], y_con[train], validation_data=(x_con[test], y_con[test]), batch_size=batch_size, epochs=nb_epochs, shuffle=True, callbacks = [callback])
    ...
    score = model.evaluate(x_con[test], y_con[test], verbose=0)
...
writePickle(acc_per_fold, "folds/acc_per_fold_"+str(name))
writePickle(loss_per_fold, "folds/loss_per_fold_"+str(name))
writePickle(selections, "folds/selections_"+str(name))
